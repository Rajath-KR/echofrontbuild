...
const CommentSection: React.FC<CommentSectionProps> = ({ postId, onClose }) => {
  ...
  const fetchComments = async () => {
    try {
      ...
      const response = await fetch(`http://13.53.231.92:8000/comments/post/${postId}`);
      if (response.ok) {
  ...
  const handleSubmitComment = async () => {
    ...
    try {
      const response = await fetch('http://13.53.231.92:8000/comments', {
        method: 'POST',
        headers: {
  ...
  const handleDeleteComment = async (commentId: string) => {
    try {
      const response = await fetch(`http://13.53.231.92:8000/comments/${commentId}`, {
        method: 'DELETE',
      });
            ...
            <div key={comment._id} className="flex gap-3 group">
              <Avatar className="h-8 w-8 flex-shrink-0">
                {comment.userId?.profileImg ? (
                  <AvatarImage
                    src={`http://13.53.231.92:8000/Images/${comment.userId.profileImg}`}
        ...
        <div className="flex items-center gap-2">
          {profileData[0]?.profileImg && (
            <Avatar className="h-8 w-8">
              <AvatarImage
                src={`http://13.53.231.92:8000/Images/${profileData[0].profileImg}`}
                alt="Your avatar"
              />
              <AvatarFallback>

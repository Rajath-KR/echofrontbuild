...
const Profile = () => {
  ...
  const fetchProfiles = async () => {
    try {
      setLoading(true);
      const response = await fetch('http://13.53.231.92:8000/profile');
  ...
  useEffect(() => {
    const fetchPosts = async () => {
      try {
        const response = await fetch('http://13.53.231.92:8000/post');
        if (response.ok) {
          const result = await response.json();
  ...
  const handleDeleteProfile = async (id: string) => {
    try {
      const response = await fetch(`http://13.53.231.92:8000/profile/${id}`, {
        method: 'DELETE',
      });
  ...
  const getImageUrl = (imagePath: string) => {
    return `http://13.53.231.92:8000/Images/${imagePath}?v=${imageVersion}`;
  };
                          ...
                          {postLists.map(
                            (item, index) =>
                              item.postImg && (
                                ...
                                >
                                  <img
                                    src={`http://13.53.231.92:8000/Images/${item.postImg}`}
                                    alt=""
            ...
            <div className="flex flex-col md:flex-row">
              <div className="md:w-2/3 h-96 bg-black flex items-center justify-center">
                {selectedPost.postImg && (
                  <img
                    src={`http://13.53.231.92:8000/Images/${selectedPost.postImg}`}
                    alt="Post"

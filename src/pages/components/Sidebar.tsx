...
export function Sidebar({ className }: SidebarProps) {
  ...
  const fetchStats = async () => {
    try {
      const postsResponse = await fetch('http://13.53.231.92:8000/post');
      if (postsResponse.ok) {
        const postsResult = await postsResponse.json();
        setStats((prev) => ({ ...prev, posts: postsResult.data.length }));
      }

      const followersResponse = await fetch('http://13.53.231.92:8000/followers');
      if (followersResponse.ok) {
        const followersResult = await followersResponse.json();
        setStats((prev) => ({ ...prev, followers: followersResult.data.length }));
      }

      const followingResponse = await fetch('http://13.53.231.92:8000/following');
            ...
            profileData.map((profile) => (
              ...
              >
                <Avatar className="h-24 w-24 bg-[#3b3b47] border-2 border-gray-600 ring ring-blue-500/30">
                  {profile.profileImg ? (
                    <AvatarImage
                      src={`http://13.53.231.92:8000/Images/${profile.profileImg}`}

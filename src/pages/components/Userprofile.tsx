...
const UserProfile = () => {
  ...
  const fetchUserProfile = async () => {
    try {
      setLoading(true);
      const response = await fetch(`http://13.53.231.92:8443/follow/user/${username}?currentUserId=${currentUserId}`);
      if (!response.ok) {
  ...
  const handleFollow = async () => {
    try {
      const response = await fetch(`http://13.53.231.92:8443/follow/follow/${userData?._id}`, {
        method: 'POST',
  ...
  const handleUnfollow = async () => {
    try {
      const response = await fetch(`http://13.53.231.92:8443/follow/unfollow/${userData?._id}`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ followerId: currentUserId })
      });
  ...
  const getImageUrl = (imagePath: string) => {
    return `http://13.53.231.92:8443/Images/${imagePath}`;
  };
                    ...
                    {posts.length > 0 ? (
                      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
                        {posts.map((item) => (
                          ...
                          >
                            <img
                              src={`http://13.53.231.92:8443/Images/${item.postImg}`}
                              alt=""
